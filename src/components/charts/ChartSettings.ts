...
export const ChartOptionsSettings = (format: LocalizeModel): ChartOptions<'line'> => {
    return {
        layout: {},
        scales: {
            y: {
                ...
                callbacks: {
                    ...
                    title: function (tooltipItem) {
                        ...
                        return format.currency === 'EUR' || format.currency === 'USD' ?
                            ToCurrency(amount, format.decimals, format.abbr, format.currency) :
                            format.currency === '%' ? ToPercent(amount, format.decimals) :
                                ToCoinCurrency(amount, format.decimals, format.abbr) + ` ${format.currency}`
                    }
                }
            }
        },
...
export const ChartDataSettings = (data: ChartDataModel[]): ChartData<'line'> => { 
    return {
